...
class Billing extends Component {
  ...
  renderActivities() {
    ...
    for( let i = 0; i < this.state.activityRows; i++) {
      activities.push(
        <Form.Group key={i}>
          <Form.Input label='W.S.NO' placeholder='W.S.NO' width={3} />
          <Form.Input label='Lorry NO' placeholder='Lorry NO' width={4} />
          <Form.Select label='Jattu' options={ getMasters('jattus') } placeholder='Jattu' width={4} />
          <Form.Input label='Weight in Tons' placeholder='Weight' width={3} />
